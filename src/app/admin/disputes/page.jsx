...
import { AlertTriangle, CheckCircle, Truck, RefreshCw, Send, Terminal, ChevronRight, X, ExternalLink } from 'lucide-react';
const MOCK_DISPUTES = [
    {
        id: 'disp-001',
        order_id: 'ORD-8241',
...
export default function AdminDisputesPage() {
                ...
                try {
                    ...
                    localOrders.forEach((o) => {
                        if (o.escrow_status === 'disputed' || o.escrow_status === 'refunded' || (o.escrow_status === 'released' && o.status === 'delivered')) {
                            const exists = mergedDisputes.some((d) => d.order_id === o.id || d.id === 'disp_local_' + o.id);
                            if (!exists) {
    ...
    function SyntaxHighlightedJSON({ data }) {
        if (!data)
            return <span className={styles.consoleText}>No response payload received yet. Trigger the webhook above.</span>;
        const jsonString = JSON.stringify(data, null, 2);
        const lines = jsonString.split('\n');
        return (<pre style={{ margin: 0, whiteSpace: 'pre-wrap', wordBreak: 'break-all' }}>
        {lines.map((line, idx) => {
                ...
                return (<div key={idx} className={styles.consoleText}>
              {line}
            </div>);
            })}
      </pre>);
    ...
    return (<>
      ...
      <div className={styles.pageContent}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: '1.5rem', alignItems: 'start' }}>
          ...
          <div className={styles.panel}>
            ...
            {loading ? (<div style={{ padding: '3rem', textAlign: 'center', color: 'var(--d-t3)' }}>Loading claims...</div>) : disputes.length === 0 ? (<div className={styles.empty}>
                ...
                <tbody>
                  {disputes.map((dispute) => (<tr key={dispute.id}>
                      <td>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              ...
              <div style={{ padding: '1.25rem' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: '0.75rem', marginBottom: '0.75rem' }}>
                  <div className={styles.fGroup} style={{ marginBottom: 0 }}>
                    ...
                    <input type="text" className={styles.fInput} style={{ fontFamily: 'monospace', fontSize: '0.8rem' }} value={simOrderId} onChange={e => setSimOrderId(e.target.value)} placeholder="e.g. ORD-8241"/>
                  </div>
                  <div className={styles.fGroup} style={{ marginBottom: 0 }}>
                    <label className={styles.fLabel} style={{ color: 'var(--color-cyan)' }}>Carrier</label>
                    <select className={styles.fSelect} style={{ fontSize: '0.8rem' }} value={simCarrier} onChange={e => {
            ...
            setSimTracking(`TRK-${e.target.value.toUpperCase()}-${simOrderId.replace('ORD-', '')}`);
                ...
                <div className={styles.fGroup} style={{ marginBottom: '1rem' }}>
                  ...
                  <input type="text" className={styles.fInput} style={{ fontFamily: 'monospace', fontSize: '0.8rem' }} value={simTracking} onChange={e => setSimTracking(e.target.value)}/>
                </div>

                <button onClick={runDeliveryWebhookSimulation} className={styles.btnPrimary} style={{
            width: '100%',
            ...
            <div className={styles.panel} style={{ border: '1px solid var(--d-border)' }}>
              <div className={styles.panelHeader} style={{ background: '#090d16', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                ...
                {responseStatus && (<span className={styles.panelBadge} style={{
            ...
            <div className={styles.panel} style={{ border: '1px solid var(--d-border)' }}>
              ...
              <div style={{ padding: '0.85rem', background: '#020617', minHeight: '90px', borderRadius: '0 0 16px 16px', overflowY: 'auto', maxHeight: '150px' }}>
                {logs.length === 0 ? (<div style={{ color: '#64748b', fontSize: '0.75rem', fontFamily: 'monospace' }}>No event logs logged.</div>) : (logs.map((log, index) => (<div key={index} className={styles.consoleLogLine}>
                      <span className={styles.consoleTime}>[{log.timestamp}]</span>
                      <span className={log.type === 'info' ? styles.consoleInfo :
      ...
      {selectedDispute && (<div className={styles.overlayRight} onClick={() => setSelectedDispute(null)}>
          <div className={styles.drawer} onClick={e => e.stopPropagation()}>
            ...
            <div className={styles.drawerBody}>
              ...
              <div className={styles.fGroup}>
                ...
                <div style={{ fontSize: '0.85rem', color: 'var(--d-t2)', lineHeight: 1.6, background: 'var(--d-hover)', border: '1px solid var(--d-border)', borderRadius: '8px', padding: '1rem' }}>
                  ...
                  {selectedDispute.evidence_url && (<div style={{ marginTop: '0.75rem', display: 'flex', gap: 6 }}>
                      <a href="#" onClick={e => { e.preventDefault(); alert("Viewing evidence image attachment in full scale..."); }} style={{ fontSize: '0.75rem', color: 'var(--color-indigo)', display: 'inline-flex', alignItems: 'center', gap: 4, fontWeight: 600 }}>
              ...
              <div className={styles.fGroup}>
                ...
                <textarea className={styles.fTextarea} rows={4} placeholder="Provide reasoning for the arbitration ruling, e.g. 'Evidence review shows damaged cuffs. Partial store credit was offered but buyer requests refund. Approved full return.'" value={noteInput} onChange={e => setNoteInput(e.target.value)} disabled={['resolved_released', 'resolved_refunded'].includes(selectedDispute.status)}/>
              </div>

              {/* Status Banner */}
              {['resolved_released', 'resolved_refunded'].includes(selectedDispute.status) && (<div style={{
                    padding: '1rem',
                    borderRadius: '8px',
            ...
            <div className={styles.drawerFooter}>
              ...
              {['open', 'under_review'].includes(selectedDispute.status) && (<>
                  <button className={styles.glowBtnRefund} onClick={() => handleResolve(selectedDispute.id, 'refund')}>
                    Approve Buyer Refund
                  </button>
                  <button className={styles.glowBtnRelease} onClick={() => handleResolve(selectedDispute.id, 'release')}>
                    Release to Vendor
                  </button>
                </>)}
            </div>
