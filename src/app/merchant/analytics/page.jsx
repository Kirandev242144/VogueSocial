...
const CustomTooltip = ({ active, payload, label }) => {
    if (!active || !payload?.length)
        return null;
    return (<div style={{ background: '#1a2035', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, padding: '10px 14px', minWidth: 120 }}>
      <div style={{ color: 'rgba(255,255,255,0.4)', fontSize: '0.72rem', marginBottom: 6 }}>{label}</div>
      {payload.map((p, i) => (<div key={i} style={{ color: p.color, fontWeight: 700, fontSize: '0.88rem' }}>{p.name === 'rev' ? '$' : ''}{p.value} {p.name === 'rev' ? 'revenue' : p.name === 'orders' ? 'orders' : ''}</div>))}
    </div>);
};
    ...
    return (<>
      <div className={styles.topbar}>
        ...
        <div className={styles.topbarRight}>
          {RANGES.map(r => (<button key={r} onClick={() => setRange(r)} style={{ padding: '0.45rem 0.9rem', borderRadius: 7, border: '1px solid',
      ...
      <div className={styles.pageContent}>
        ...
        <div className={styles.kpiGrid} style={{ marginBottom: '1.25rem' }}>
          {METRICS.map(m => {
            const Icon = m.icon;
            return (<div key={m.label} className={styles.kpiCard}>
        ...
        <div className={styles.panel} style={{ marginBottom: '1rem' }}>
          ...
          <div style={{ padding: '1.25rem' }}>
            ...
            <div style={{ display: 'flex', gap: '1.5rem', paddingTop: '0.75rem', borderTop: `1px solid ${isDark ? 'rgba(255,255,255,0.04)' : '#e5e7eb'}` }}>
              {[{ color: c1, label: 'Revenue' }, { color: c2, label: 'Orders' }].map(l => (<div key={l.label} style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
            ...
            <div style={{ padding: '1.25rem', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              <ResponsiveContainer width="100%" height={160}>
                <PieChart>
                  ...
                  <Tooltip formatter={(v, n) => [`${v}%`, n]} contentStyle={{ background: '#1a2035', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, fontSize: '0.8rem' }}/>
                </PieChart>
              </ResponsiveContainer>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', width: '100%', marginTop: '0.5rem' }}>
                {CATEGORY.map((c, i) => (<div key={c.name} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
