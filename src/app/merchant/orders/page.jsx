    ...
    useEffect(() => {
        ...
        if (localOrdersStr) {
            try {
                ...
                const formatted = localOrders.map((o) => ({
      ...
      <div className={styles.pageContent}>
        <div className={styles.panel}>
          <div className={styles.tabs}>
            {TABS.map(t => (<button key={t} className={`${styles.tab} ${activeTab === t ? styles.tabActive : ''}`} onClick={() => setActiveTab(t)}>
                {t}
          ...
          <table className={styles.table}>
            ...
            <tbody>
              {filtered.length === 0 ? (<tr><td colSpan={7}><div className={styles.empty}><Package size={36} className={styles.emptyIcon}/><div className={styles.emptyTitle}>No orders found</div></div></td></tr>) : filtered.map(o => {
            const s = SC[o.status];
            const Icon = s.icon;
            ...
            return (<tr key={o.id}>
                    ...
                    : o.status === 'pending' ? (<div className={styles.trackingRow}>
                        ...
                        </div>) : <span style={{ color: 'var(--d-t4)', fontSize: '0.78rem' }}>—</span>}
                    </td>
      ...
      {selected && (<div className={styles.overlay} onClick={() => setSelected(null)}>
          <div className={styles.modal} onClick={e => e.stopPropagation()}>
            ...
            <div className={styles.modalBody}>
              {[['Product', selected.product], ['Customer', selected.customer], ['Email', selected.email], ['Ship To', selected.address], ['Size', selected.size], ['Quantity', `${selected.qty}`], ['Amount', selected.amount], ['Date', selected.date], ['Tracking', trackingMap[selected.id] || selected.trackingId || 'Not assigned']].map(([l, v]) => (<div key={l} style={{ display: 'flex', justifyContent: 'space-between', padding: '0.6rem 0', borderBottom: '1px solid var(--d-border2)' }}>
